...
public class KeypleTest_ObservablePoTransaction implements ReaderObserver {
	...
	public void operatePoTransaction() {
		...
		try {
			...
			ReadRecordsCmdBuild poReadRecordCmd_06 = new ReadRecordsCmdBuild(PoRevision.REV3_1, (byte) 0x01, true, (byte) 0x06, (byte) 0x01);
//			ReadRecordsCmdBuild poReadRecordCmd_06 = new ReadRecordsCmdBuild(PoRevision.REV3_1, (byte) 0x01, true, (byte) 0x06, (byte) 0x78);
			// Read first record of SFI 08h - for 15h bytes
			ReadRecordsCmdBuild poReadRecordCmd_08 = new ReadRecordsCmdBuild(PoRevision.REV3_1, (byte) 0x01, true, (byte) 0x08, (byte) 0x01);
//			ReadRecordsCmdBuild poReadRecordCmd_08 = new ReadRecordsCmdBuild(PoRevision.REV3_1, (byte) 0x01, true, (byte) 0x08, (byte) 0x15);
			...
			// Step 2
			System.out.println("========= PO Transaction ======= Opening ============================");
			// Read first record of SFI 08h - for 15h bytes
			byte debitKeyIndex = 0x03;
			// Open Session for the debit key #1 - with read of the first record of the cyclic EF of SFI 0Ah
			OpenSessionCmdBuild poOpenSession = new OpenSessionCmdBuild(poTransaction.getRevision(), debitKeyIndex, poTransaction.sessionTerminalChallenge, (byte) 0x0A, (byte) 0x01);
			poTransaction.processOpening(poOpenSession, filesToReadInSession);
//			poTransaction.processOpening(poOpenSession, null);
			
			// Step 3
			System.out.println("========= PO Transaction ======= Continuation =======================");
			poTransaction.processProceeding(filesToReadInSession);
			
			// Step 4
			System.out.println("========= PO Transaction ======= Closing ============================");
//			poTransaction.processClosing(filesToReadInSession, poAnticipatedResponseInsideSession, poReadRecordCmd_06); // TODO - to complete support of poAnticipatedResponseInsideSession
			poTransaction.processClosing(null, null, poReadRecordCmd_06);

			if (poTransaction.isSuccessful()) {
				System.out.println("========= PO Transaction ======= SUCCESS !!!!!!!!!!!!!!!!!!!!!!!!!!!!");
			} else {
				System.out.println("========= PO Transaction ======= ERROR !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");			
			}
	...
	public static void main(String[] args) throws TestSettingException {		
		...
		for(int i=0; i<nbPlugins; i++){
			...
			try {
				...
				System.out.println("Reader number / Reader name                                     / SE presence / Assignment");
				...
				for(int u=0; u<readers.size();u++){
					...
					System.out.printf("%-16s%-50s%-14s%-10s\n", "    "+u, reader.getName(), "    " + ((reader.isSEPresent()) ? "YES" : "NO"), "  "+readerAssignment);
				}	
		...
		try {
			...
			if (poReaderRef == csmReaderRef) throw new TestSettingException("PO & CSM readers conflit!");
