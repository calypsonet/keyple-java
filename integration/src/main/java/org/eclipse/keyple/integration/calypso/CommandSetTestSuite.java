...
public class CommandSetTestSuite {
    ...
    /*
     * @Test public void testWriteCounter() {
     * 
     * try {
     * 
     ...
     * 
     * ReadRecordsRespPars readRecordsRespPars = readRecords((CalypsoPo) poData.getMatchingSe(),
     * poData.getCountersFileData().getSfi(), (byte) 0x01, true); byte[] updatedCounterData =
     ...
     * 
     * readRecordsRespPars = readRecords((CalypsoPo) poData.getMatchingSe(), (byte)
     * (poData.getSimulatedCountersFileData().getSfi() + i), (byte) 0x01, true);
     * 
     ...
     * } catch (Exception e) {
     * 
     * Assertions.fail("Exception caught: " + e.getMessage()); e.printStackTrace(); }
     * 
     * }
     * 
     * @Test public void testDecreaseCounter() {
     * 
     * try {
     * 
     * PoFileStructureInfo poData = selectPO();
     ...
     * if (counterValue > 0) {
     * 
     * int valueToDecrement = counterValue / 2; decreaseCounter((CalypsoPo) poData.getMatchingSe(),
     * poData.getCountersFileData().getSfi(), (byte) (i + 1), valueToDecrement);
     * 
     * readRecordsRespPars = readRecords((CalypsoPo) poData.getMatchingSe(),
     * poData.getCountersFileData().getSfi(), (byte) 0x01, true);
     * 
     * byte[] updatedCounters = readRecordsRespPars.getRecords().get(1); int finalValue =
     * getCounterValueFromByteArray(updatedCounters, i + 1);
     * 
     ...
     * (poData.getSimulatedCountersFileData().getSfi() + i), (byte) 0x01, true);
     * 
     * updatedCounters = readRecordsRespPars.getRecords().get(1); finalValue =
     * getCounterValueFromByteArray(updatedCounters, 1);
     * 
     * Assertions.assertEquals(counterValue - valueToDecrement, finalValue); } } }
     * 
     * } catch (Exception e) {
     ...
     * 
     * 
     * @Test public void testIncreaseCounter() {
     * 
     * try {
     * 
     * PoFileStructureInfo poData = selectPO();
     ...
     * for (int i = 0; i < (poData.getCountersFileData().getRecSize() / 3); i++) {
     * 
     * int counterValue = getCounterValueFromByteArray(initialCounterData, i + 1); int maxValue =
     * 0xFFFFFF;
     * 
     * if (counterValue < maxValue) {
     * 
     * int valueToIncrement = (maxValue - counterValue) / 2; increaseCounter((CalypsoPo)
     * poData.getMatchingSe(), poData.getCountersFileData().getSfi(), (byte) (i + 1),
     * valueToIncrement);
     * 
     * readRecordsRespPars = readRecords((CalypsoPo) poData.getMatchingSe(),
     * poData.getCountersFileData().getSfi(), (byte) 0x01, true);
     ...
     * try {
     * 
     * PoFileStructureInfo poData = selectPO(); Boolean readOneRecordFlag = false; PoRevision
     * poRevision = (new PoTransaction(TestEngine.poReader, (CalypsoPo) poData.getMatchingSe(),
     * TestEngine.samReader, null)).getRevision();
     * 
     * 
     ...
     * Arrays.fill(recordData, (byte) (i + 1)); recordsData.add(recordData);
     * 
     * updateRecord((CalypsoPo) poData.getMatchingSe(), poData.getContractFileData().getSfi(),
     * (byte) (i + 1), recordData); }
     * 
     * if (poRevision == PoRevision.REV2_4) { readOneRecordFlag = true; }
     * 
     * ReadRecordsRespPars readRecordsRespPars = readRecords((CalypsoPo) poData.getMatchingSe(),
     ...
     * Assertions.assertArrayEquals(recordsData.get(0), readRecordsRespPars.getRecords().get(1));
     * 
     * } else {
     * 
     * for (int i = 0; i < poData.getContractFileData().getRecNumb(); i++) {
     * 
     * Assertions.assertArrayEquals(recordsData.get(i), readRecordsRespPars.getRecords().get(i +
     * 1)); }
     * 
     * }
     * 
    ...
    /*
     * @Test public void testUpdateAndDecreaseCounterSingleSession() {
     * 
     * try {
     * 
     * PoFileStructureInfo poData = selectPO();
     * 
     * PoTransaction poTransaction = new PoTransaction(TestEngine.poReader, (CalypsoPo)
     * poData.getMatchingSe(), TestEngine.samReader, null);
     * 
     * String genericCounterData =
     * "00000A 000100 000B00 010000 0C0000 0000B0 00C000 0F0000 00000D 0000";
     * 
     ...
     * counterData, String.format("SFI=%02X, recnbr=1", poData.getCountersFileData().getSfi()));
     * 
     * poTransaction.prepareDecreaseCmd(poData.getCountersFileData().getSfi(), (byte) 0x01, 0x01,
     * String.format("SFI=%02X, index=1, decvalue=1", poData.getCountersFileData().getSfi()));
     * 
     * poTransaction.processPoCommands(ChannelState.KEEP_OPEN);
     * 
     * poTransaction.processClosing(CommunicationMode.CONTACTLESS_MODE, ChannelState.KEEP_OPEN);
     ...
     * Assertions.fail("Exception caught: " + e.getMessage()); e.printStackTrace(); } }
     * 
     * @Test public void testUpdateAndIncreaseCounterSingleSession() {
     * 
     * try {
     * 
     * PoFileStructureInfo poData = selectPO();
     ...
     * counterData, String.format("SFI=%02X, recnbr=1", poData.getCountersFileData().getSfi()));
     * 
     * poTransaction.prepareIncreaseCmd(poData.getCountersFileData().getSfi(), (byte) 0x01, 0xFF,
     * String.format("SFI=%02X, index=1, decvalue=255", poData.getCountersFileData().getSfi()));
     * 
     * poTransaction.processPoCommands(ChannelState.KEEP_OPEN);
     * 
     * poTransaction.processClosing(CommunicationMode.CONTACTLESS_MODE, ChannelState.KEEP_OPEN);
