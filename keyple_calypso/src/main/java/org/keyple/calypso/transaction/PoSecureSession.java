...
public class PoSecureSession {
    ...
    /**
     * The PO Transaction State defined with the elements: ‘IOError’, ‘SEInserted’
     * and ‘SERemoval’.
     */
    ...
    public PoRevision poRevision = PoRevision.REV3_1;//PoCommandBuilder.defaultRevision; // TODO => add a getter
     ...
     *            default KIF index
     *            
     *            TODO => to replace the "defaultKeyIndex" byte parameter by a "csmSetting" Map<String> (or ByteArray, or List<Byte>) parameter 
     */
...
//    }
    ...
    public SeResponse processIdentification(byte[] poAid, List<SendableInSession> poCommandsOutsideSession)
        ...
        List<ApduRequest> poApduRequestList = this.getApduRequestListFromSendableInSessionListTo(poCommandsOutsideSession);
		...
		System.out.println("\t========= Identification === Selected DF Name : " + DatatypeConverter.printHexBinary(poCalypsoInstanceAid));
        poCalypsoInstanceSerial = poFciRespPars.getApplicationSerialNumber();
		System.out.println("\t========= Identification === Calypso Serial Number : " + DatatypeConverter.printHexBinary(poCalypsoInstanceSerial));
				
        // Define CSM Select Diversifier command
        ApduCommandBuilder selectDiversifier = new SelectDiversifierCmdBuild(this.csmRevision, poCalypsoInstanceSerial);
        csmApduRequestList.add(selectDiversifier.getApduRequest());
		System.out.println("\t========= Identification === Generate CSM cmd request - Select Diversifier : " + DatatypeConverter.printHexBinary(selectDiversifier.getApduRequest().getbytes()));
        
        // Define CSM Get Challenge command
        ApduCommandBuilder csmGetChallenge = new CsmGetChallengeCmdBuild(this.csmRevision, (byte) 0x04);
        csmApduRequestList.add(csmGetChallenge.getApduRequest());
		System.out.println("\t========= Identification === Generate CSM cmd request - Get Challenge : " + DatatypeConverter.printHexBinary(csmGetChallenge.getApduRequest().getbytes()));
		...
		System.out.println("\t\tDEBUG ##### csmApduResponseList.size() : " + csmApduResponseList.size());
        if (csmApduResponseList.size()==2) {
        	// TODO => check that csmApduResponseList.get(1) has the right length (challenge + status)
            CsmGetChallengeRespPars csmChallengePars = new CsmGetChallengeRespPars(csmApduResponseList.get(1));
    		System.out.println("\t========= Identification === Parse CSM cmd response - Select Diversifier : " + DatatypeConverter.printHexBinary(csmChallengePars.getApduResponse().getbytes()));
            sessionTerminalChallenge = csmChallengePars.getChallenge();
    		System.out.println("\t========= Identification === Terminal Challenge : " + DatatypeConverter.printHexBinary(sessionTerminalChallenge));
        } else {
...
//        List<ApduRequest> poApduRequestList = this.getApduRequestListFromSendableInSessionListTo(poCommandsInsideSession);
        ...
        if ((poCommandsInsideSession != null) && !poCommandsInsideSession.isEmpty()) {
        	poApduRequestList.addAll(this.getApduRequestListFromSendableInSessionListTo(poCommandsInsideSession));
        }
       
        // Transfert PO commands
		System.out.println("\t========= Opening ========== Transfert PO commands");
        SeRequest poRequest = new SeRequest(poCalypsoInstanceAid, poApduRequestList, keepChannelOpen);    
        SeResponse poResponse = poReader.transmit(poRequest);
        ...
        // TODO => check that csmApduResponseList.get(1) has the right length (challenge + status)
        OpenSessionRespPars poOpenSessionPars = new OpenSessionRespPars(poApduResponseList.get(0), poRevision); // first item of poApduResponseList
		System.out.println("\t========= Opening ========== Parse PO cmd response - Open Session : " + DatatypeConverter.printHexBinary(poOpenSessionPars.getApduResponse().getbytes()));
        sessionCardChallenge = poOpenSessionPars.getPoChallenge();
		System.out.println("\t========= Opening ========== WRONG Card Challenge : " + DatatypeConverter.printHexBinary((byte[]) sessionCardChallenge));
		
		
        // HACK - OpenSessionRespPars.getPoChallenge() ne retourne pas la bonne valeur de PO challenge
		// TODO - corriger => OpenSessionRespPars.getPoChallenge()
		sessionCardChallenge = DatatypeConverter.parseHexBinary(DatatypeConverter.printHexBinary(poOpenSessionPars.getApduResponse().getbytes()).substring(0, 4*2)); // HACK
		System.out.println("\t========= Opening ========== HACKED Card Challenge : " + DatatypeConverter.printHexBinary((byte[]) sessionCardChallenge));
		
    	// Build "Digest Init" command from PO Open Session
        byte kif = poOpenSessionPars.getSelectedKif();
		System.out.println("\t========= Opening ========== PO KIF : " + DatatypeConverter.printHexBinary(new byte[] { (byte) kif}));
		System.out.println("\t========= Opening ========== PO KVC : " + DatatypeConverter.printHexBinary(new byte[] { (byte) poOpenSessionPars.getSelectedKvc()}));
        if (kif == (byte) 0xFF) {
        ...
        ApduCommandBuilder digestInit = new DigestInitCmdBuild(csmRevision, false,
                poRevision.equals(PoRevision.REV3_2), defaultKeyIndex, kif, poOpenSessionPars.getSelectedKvc(),
                poOpenSessionPars.getRecordDataRead());
		System.out.println("\t========= Opening ========== Generate CSM cmd request - Digest Init : " + DatatypeConverter.printHexBinary(digestInit.getApduRequest().getbytes()));
        ...
        if ((poCommandsInsideSession != null) && !poCommandsInsideSession.isEmpty() && (poCommandsInsideSession.size()>1)) {
        	// TODO => rajouter un contr�le afin de v�rifier que poApduResponseList a m�me taille que poApduRequestList
        	for(int i = 1; i < poApduRequestList.size(); i++) { // The loop starts after the Open Session for the first command send in session
            	...
            	csmApduRequestList.add((new DigestUpdateCmdBuild(csmRevision, false, poApduRequestList.get(i).getbytes())).getApduRequest());
            	// Build "Digest Update" command for each PO APDU Response
            	System.out.println("\t========= Opening ==WRONG=== Generate CSM cmd request - Digest Update for PO response : " + DatatypeConverter.printHexBinary((new DigestUpdateCmdBuild(csmRevision, false, poApduResponseList.get(i).getbytes())).getApduRequest().getbytes()));
//            	csmApduRequestList.add((new DigestUpdateCmdBuild(csmRevision, false, poApduResponseList.get(i).getbytes())).getApduRequest()); //TODO => this is the rigth command, to fix ApduResponse.getbytes
            	byte[] additionOfGetBytesAndGetStatusCode = ArrayUtils.addAll(poApduResponseList.get(i).getbytes(), poApduResponseList.get(i).getStatusCode());
//        		System.out.println("\t\tDEBUG ##### csmApduResponseList.size() : " + DatatypeConverter.printHexBinary(additionOfGetBytesAndGetStatusCode));
            	System.out.println("\t========= Opening ==HACK==== Generate CSM cmd request - Digest Update for PO response : " + DatatypeConverter.printHexBinary((new DigestUpdateCmdBuild(csmRevision, false, additionOfGetBytesAndGetStatusCode)).getApduRequest().getbytes()));
            	csmApduRequestList.add(((new DigestUpdateCmdBuild(csmRevision, false, additionOfGetBytesAndGetStatusCode)).getApduRequest())); // HACK
            }
        }
...
//    private List<ApduRequest> changeSendableInSessionTabToListApdu(SendableInSession[] poCommandsInsideSession) {
    private List<ApduRequest> getApduRequestListFromSendableInSessionListTo(List<SendableInSession> poCommandsInsideSession) {
...
//                retour.add(cmd.getAPDURequest()); TODO => suppress all methods getAPDURequest() from SendableInSession & most of PoCommandBuilder extensions
    ...
    public SeResponse processProceeding(List<SendableInSession> poCommandsInsideSession)
        ...
        for(int i = 0; i < poApduRequestList.size(); i++) {        	
        	...
        	csmApduRequestList.add((new DigestUpdateCmdBuild(csmRevision, false, poApduRequestList.get(i).getbytes())).getApduRequest());
        	// Build "Digest Update" command for each PO APDU Response
//        	System.out.println("\t========= Continuation ===== Generate CSM cmd request - Digest Update for PO response : " + DatatypeConverter.printHexBinary((new DigestUpdateCmdBuild(csmRevision, false, poApduResponseList.get(i).getbytes())).getApduRequest().getbytes()));
        	System.out.println("\t==WRONG== Continuation ===== Generate CSM cmd request - Digest Update for PO response : " + DatatypeConverter.printHexBinary((new DigestUpdateCmdBuild(csmRevision, false, poApduResponseList.get(i).getbytes())).getApduRequest().getbytes()));
//        	csmApduRequestList.add((new DigestUpdateCmdBuild(csmRevision, false, poApduResponseList.get(i).getbytes())).getApduRequest()); //TODO => this is the rigth command, to fix ApduResponse.getbytes
        	byte[] additionOfGetBytesAndGetStatusCode = ArrayUtils.addAll(poApduResponseList.get(i).getbytes(), poApduResponseList.get(i).getStatusCode());
//    		System.out.println("\t\tDEBUG ##### csmApduResponseList.size() : " + DatatypeConverter.printHexBinary(additionOfGetBytesAndGetStatusCode));
        	System.out.println("\t==HACK=== Continuation ===== Generate CSM cmd request - Digest Update for PO response : " + DatatypeConverter.printHexBinary((new DigestUpdateCmdBuild(csmRevision, false, additionOfGetBytesAndGetStatusCode)).getApduRequest().getbytes()));
        	csmApduRequestList.add(((new DigestUpdateCmdBuild(csmRevision, false, additionOfGetBytesAndGetStatusCode)).getApduRequest())); // HACK
...
// TODO - pr�voir une variante pour enchainer plusieurs session d'affil�es (la commande de ratification �tant un nouveau processOpening)
        ...
        if ((poCommandsInsideSession != null) && !poApduRequestList.isEmpty()) {
        	if (poApduRequestList.size() == poAnticipatedResponseInsideSession.size()) {
                ...
                for(int i = 0; i < poApduRequestList.size(); i++) {   
                	...
                	// Build "Digest Update" command for each PO APDU Request
                	csmApduRequestList_1.add((new DigestUpdateCmdBuild(csmRevision, false, poApduRequestList.get(i).getbytes())).getApduRequest());
                	// Build "Digest Update" command for each "ANTICIPATED" PO APDU Response
//                	csmApduRequestList_1.add((new DigestUpdateCmdBuild(csmRevision, false, poApduResponseList_1.get(i).getbytes())).getApduRequest());
                	csmApduRequestList_1.add((new DigestUpdateCmdBuild(csmRevision, false, poAnticipatedResponseInsideSession.get(i).getbytes())).getApduRequest());
                }
        	} else {
        ...
        //csmApduRequestList_1.add((new DigestCloseCmdBuild(csmRevision, poRevision.equals(PoRevision.REV3_2) ? (byte) 0x08 : (byte) 0x04)).getApduRequest());
        DigestCloseCmdBuild digestClose = new DigestCloseCmdBuild(csmRevision, poRevision.equals(PoRevision.REV3_2) ? (byte) 0x08 : (byte) 0x04);
		System.out.println("\t========= Closing ========== Generate CSM cmd request - Digest Close : " + DatatypeConverter.printHexBinary(digestClose.getApduRequest().getbytes()));
		...
		System.out.println("\t\tDEBUG ##### csmApduResponseList_1.size() : " + csmApduResponseList_1.size());
        ...
        if ((csmApduResponseList_1 != null) && !csmApduResponseList_1.isEmpty()) {
        	...
        	DigestCloseRespPars respPars = new DigestCloseRespPars(csmApduResponseList_1.get(csmApduResponseList_1.size()-1)); // .getApduResponses().get(0);
        	
    		System.out.println("\t========= Closing ========== Parse CSM cmd response - Digest Close : " + DatatypeConverter.printHexBinary(respPars.getApduResponse().getbytes()));
    		System.out.println("\t\tDEBUG ##### csmApduResponseList_1.size() : " + csmApduResponseList_1.size());
        ...
        if (ratificationAsked) poApduRequestList.add(ratificationCommand.getApduRequest());
        ...
        // TODO => check that PO response is equal to anticipated PO response (that poApduResponseList equals poAnticipatedResponseInsideSession)
     
        // parse Card Signature
//        CloseSessionRespPars poCloseSessionPars = new CloseSessionRespPars(poApduResponseList.get(0), poRevision); TODO add support of poRevision parameter to CloseSessionRespPars for REV2.4 PO CLAss byte
        CloseSessionRespPars poCloseSessionPars = new CloseSessionRespPars(poApduResponseList.get(poApduResponseList.size() - ((ratificationAsked) ? 2 : 1))); // before last if ratification, otherwise last one
        sessionCardSignature = poCloseSessionPars.getSignatureLo();
        
        // Build CSM Digest Authenticate command
        ApduCommandBuilder digestAuth = new DigestAuthenticateCmdBuild(this.csmRevision, sessionCardSignature);
        ...
        if ((csmApduResponseList_2 != null) && !csmApduResponseList_2.isEmpty()) {
        	DigestAuthenticateRespPars respPars = new DigestAuthenticateRespPars(csmApduResponseList_2.get(0));
