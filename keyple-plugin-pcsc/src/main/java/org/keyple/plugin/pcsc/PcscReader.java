...
//    private Thread readerThread;
    ...
    protected PcscReader(CardTerminal terminal, String name) { // PcscReader constructor may be called only by PcscPlugin
...
//        this.readerThread.start();
        ...
        if (isSEPresent()) { //TODO si vrai ET pas vrai => retourne un SeResponse vide de manière systématique - return new SeResponse(false, fciDataSelected, apduResponseList); 
            ...
            for (ApduRequest apduRequest : seApplicationRequest.getApduRequests()) {
                ...
                try {
                	...
                	System.out.println(settings.get("protocol") + " > " + DatatypeConverter.printHexBinary(apduRequest.getbytes()));
                    apduResponseData = channel.transmit(new CommandAPDU(apduRequest.getbytes()));
                    System.out.println(settings.get("protocol") + " < " + DatatypeConverter.printHexBinary(apduResponseData.getBytes()));
    ...
    private void prepareAndConnectToTerminalAndSetChannel() throws CardException {
        ...
        if (card == null) {
    		...
    		System.out.println(terminal.getName());
        	System.out.println(settings.get("protocol") + " - connect(protocol)\t\tfrom prepareAndConnectToTerminalAndSetChannel()");
    ...
    private ApduResponse connect(byte[] aid) throws ChannelStateReaderException {
        try {
    		...
    		System.out.println(terminal.getName() + "\t\t PC/SC Select Application\t\tfrom PcscReader.connect(byte[] aid)");
        	System.out.println(settings.get("protocol") + " > " + DatatypeConverter.printHexBinary(command));

        	ResponseAPDU res = channel.transmit(new CommandAPDU(command));
        	System.out.println(settings.get("protocol") + " < " + DatatypeConverter.printHexBinary(res.getBytes()));
    ...
    private void disconnect() throws IOReaderException {
    	...
    	try {
            ...
            if (this.card != null) {
                this.channel = null;
        		System.out.println(terminal.getName());
            	System.out.println(settings.get("protocol") + " - disconnect(FALSE)\t\tfrom PcscReader.disconnect()");
...
//            		System.out.println(terminal.getName());
//                	System.out.println(settings.get("protocol") + " - disconnect()\t\tfrom PcscReader.EventThread.run()");
//                    reader.disconnect();
